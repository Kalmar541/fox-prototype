...
{
    /* Класс который отвечает за поведений бомб в игре.
     * Бомбы создаются скриптом BOSS.cs. При касании земли бомба начинает мигать и через определенное время взывается.
     * Взрыв уничтожает основание под бомбой, если игрок находится в радиусе поражения, у него отнимается жизнь.
     * Бомба, выпавшая за пределы экрана, уничтожается со сцены.
     * Бомба, которую игрок оттолкнул получает флаг is_activateExplousion, что дает ей возможность нанести урон уже BOSSу
     * Если она не долетает до BOSSа и падает вниз то подрывается на высоте( не касается земли)
     * К бомбе падающей вниз прибавляется множитель гравитации gravityScale,
     * который TODO - в зависимости от сложности ускоряет ее или замедляет.
     */
